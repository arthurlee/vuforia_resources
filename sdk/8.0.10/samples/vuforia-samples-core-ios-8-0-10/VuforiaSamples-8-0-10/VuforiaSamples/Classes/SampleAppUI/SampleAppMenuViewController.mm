...
- (void)handleTapOnWindow:(UITapGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateEnded)
    {
        ...
        // We rotate the touch coordinates to test if it hits the menu view on landscape right
        if ([[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeRight)
        {
            CGAffineTransform translateTransform = CGAffineTransformMakeTranslation(0, -self.view.frame.size.width);
            CGAffineTransform rotationTransform = CGAffineTransformMakeRotation(M_PI/2);
            CGAffineTransform customRotation = CGAffineTransformConcat(translateTransform, rotationTransform);
            location = CGPointApplyAffineTransform(location, customRotation);
        }
