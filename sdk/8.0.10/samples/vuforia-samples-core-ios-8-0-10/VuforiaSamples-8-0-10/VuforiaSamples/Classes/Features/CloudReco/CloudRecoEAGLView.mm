...
@synthesize vapp, viewController;
...
- (id)initWithFrame:(CGRect)frame
     ...
     viewController:(CloudRecoViewController *) aViewController
...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    {
        // Start the target finder if it has not already been started
    ...
    else
    {
        ...
        for (const auto* result : trackableResultList)
        {
            ...
            {
                // Stop the target finder if it has not already been stopped
                if ([viewController isVisualSearchOn])
                {
                    [viewController toggleVisualSearch];
                }
...
- (void) renderModelWithProjection: (float*) projectionMatrix withViewMatrix: (float*) viewMatrix andModelMatrix: (float*) modelMatrix
{
    ...
    SampleApplicationUtils::translatePoseMatrix(0.0f, 0.0f, kObjectScale, modelMatrix);
    SampleApplicationUtils::scalePoseMatrix(kObjectScale, kObjectScale, kObjectScale, modelMatrix);
