...
- (id) initWithFrame:(CGRect)frame appSession:(SampleApplicationSession *)app andSampleUIUpdater:(id<SampleAppsUIControl>)sampleAppsUIControl
{
    ...
    if (self)
    {
        ...
        sampleAppRenderer = [[SampleAppRenderer alloc] initWithSampleAppRendererControl:self nearPlane:.01 farPlane:10.0];
...
- (void) renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    const Vuforia::TrackableResult* vuMarkToDisplay = nullptr;
    ...
    for (const auto* result : trackableResultList)
    {
        ...
        if (result->isOfType(Vuforia::VuMarkTargetResult::getClassType()))
        {
            ...
            bool isMainVumark = ((vuMarkToDisplay == nullptr) || (vuMarkToDisplay == result));
            ...
            [self renderModelWithProjection:&projectionMatrix.data[0] withViewMatrix:&devicePoseMatrix.data[0] withModelMatrix:modelMatrix withVumarkOrigin:origin andIsMainVuMark:isMainVumark];
...
- (void) renderModelWithProjection: (float*) projectionMatrix withViewMatrix: (float*) viewMatrix withModelMatrix: (Vuforia::Matrix44F) modelMatrix withVumarkOrigin: (Vuforia::Vec2F) origin andIsMainVuMark: (BOOL) isMainVumark
{
    // OpenGL 2
    Vuforia::Matrix44F modelViewProjection;
    
    // Combine device pose (view matrix) with model matrix
    SampleApplicationUtils::multiplyMatrix(viewMatrix, &modelMatrix.data[0], &modelMatrix.data[0]);
    ...
    if (userData) {
        ...
        {
            Vuforia::Matrix44F modelViewMatrix = modelMatrix;
            [userData modelViewMatrix:modelViewMatrix forSegmentIdx:idx width:vumarkBorderSize];
            SampleApplicationUtils::multiplyMatrix(projectionMatrix, &modelViewMatrix.data[0], &modelViewProjection.data[0]);
        ...
        SampleApplicationUtils::translatePoseMatrix(translX, translY, 0.0, modelMatrix);
...
- (void) showCard {
    ...
    [UIView animateWithDuration:0.2
                     ...
                     animations:^{
                         self->cardView.frame = self->cardFrameVisible;
                     }
                     completion:^(BOOL finished){
                         self->cardViewVisible = true;
                         self->cardIdLabel.text = value;
                     }];
}
...
- (void) hideCard {
    ...
    [UIView animateWithDuration:0.2
                        ...
                        options: UIViewAnimationCurveEaseIn
                     animations:^{
                         self->cardView.frame = self->cardFrameHidden;
                     }
                     completion:^(BOOL finished){
                         self->cardViewVisible = false;
                     }];
}
