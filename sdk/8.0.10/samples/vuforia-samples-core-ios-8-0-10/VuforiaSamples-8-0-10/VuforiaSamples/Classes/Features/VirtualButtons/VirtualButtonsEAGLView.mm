...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    for (const auto* trackableResult : trackableResultList)
    {
        ...
        const auto& vbrList = imageTargetResult->getVirtualButtonResults();
        for (const auto* buttonResult : vbrList)
        {
        ...
        if (vbCounter>0)
        {
            ...
            glDrawArrays(GL_LINES, 0, imageTargetResult->getVirtualButtonResults().size()*8);
